...
class GoExplore():
    ...
    #     self.env.goto_state_sim(node_start.state_sim)
    #     node_end, done = self.explore_from(node_start)
    #     self.archive.add_node(node_end)
    #     self.latents = torch.stack([node.latent for node in self.archive.nodes])
    #     self.i_step += 1
