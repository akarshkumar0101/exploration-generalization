...
for seed in sweep_seed:
    for env_id in sweep_env_id:
        ...
        config["h"] = np.random.randint(4, 20)
        config["w"] = np.random.randint(4, 20)
        config["d"] = np.random.randint(4, 20)
        # for k in ["project", "name", "load_agent", "save_agent", "load_agent_teacher"]:
        # if config[k] is not None:
        # config[k] = config[k].format(**config)
        config["save_archive"] = f"./data/gesweep/{env_id}_{seed}.npy"
