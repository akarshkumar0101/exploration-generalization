...
class EpisodicBonus(gym.Wrapper):
    ...
    def step(self, action):
        ...
        for i, (latent, memory_) in enumerate(zip(latents, self.memories)):
            memory = list(memory_)
...
if __name__ == "__main__":
    args = parse_args()
    run_name = f"{args.obj}_{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}"
    if args.track:
        import wandb
